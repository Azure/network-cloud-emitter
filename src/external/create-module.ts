import { Binder, LibrarySymbolReference, namekey, refkey, REFKEYABLE, TO_SYMBOL, useBinder } from "@alloy-js/core";
import { FunctionSymbol, GoSymbol, NamedTypeSymbol, PackageSymbol } from "@alloy-js/go";
...
function createSymbolEntry(name: string, descriptor: Descriptor, context: InternalContext): LibrarySymbolReference {
  ...
  function getSymbol(binder: Binder | undefined) {
    ...
    return mapGet(symbols, binder, () =>
      createSymbolFromDescriptor(name, binder, descriptor, context, initializeMembers),
    );
  }
  ...
  switch (descriptor.kind) {
    ...
    case "struct":
    case "interface":
      for (const [memberName, memberDesc] of Object.entries(descriptor.members) as any) {
  ...
  switch (descriptor.kind) {
    ...
    case "field":
      ...
      return new NamedTypeSymbol(
        namekey(name),
        ownerSymbol.members,
        ownerSymbol.typeKind === "interface" ? "interface-member" : "struct-member",
        {
          binder,
          refkeys: refkey(),
          type:
...
function mapGet<T extends WeakKey, V>(map: WeakMap<T, V>, key: T | undefined): V | undefined;
function mapGet<T extends WeakKey, V>(map: WeakMap<T, V>, key: T | undefined, init: () => V): V;
function mapGet<T extends WeakKey, V>(map: WeakMap<T, V>, key: T | undefined, init?: () => V): V | undefined {
  if (key === undefined) {
